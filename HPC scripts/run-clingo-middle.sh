...
#SBATCH --job-name="ClingoMiddleInstances"
...
echo "Starting ASP solving on ITC2021 Middle instances with all configurations..."
...
# Loop through all ITC2021 Middle XML files (1-15)
for i in {1..15}; do
    xml_file="${INSTANCE_DIR}/ITC2021_Middle_${i}.xml"
    
    if [ -f "$xml_file" ]; then
        echo "Processing ITC2021_Middle_${i}..."
        
        # Generate facts from XML
        python3 itc2021_fact_parser.py "$xml_file" "facts_middle${i}.lp"
        ...
        for config_name in "${!configs[@]}"; do
            ...
            mkdir -p "results/middle"
            
            # Run Clingo with 10-minute timeout and save to JSON
            timeout 600s clingo encodingV4.lp "facts_middle${i}.lp" \
                ${configs[$config_name]} \
                --parallel-mode=$OMP_NUM_THREADS \
                --opt-mode=optN \
                --outf=2 > "results/middle/middle${i}_${config_name}.json"
            
            # Check if timeout occurred
            if [ $? -eq 124 ]; then
                echo "  Timeout occurred for Middle_${i} with $config_name"
            fi
        done
        
        # Clean up facts file
        rm "facts_middle${i}.lp"
    else
        echo "Warning: $xml_file not found"
    fi
done

echo "All ITC2021 Middle instances completed."